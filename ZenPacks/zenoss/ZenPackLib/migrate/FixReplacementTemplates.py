                ...
                for templ in templs:
                    if templ.endswith('-replacement') or templ.endswith('-addition'):
                        ...
                        if base in availtempls:
                            LOG.info("Removing template %s from %s.zDeviceTemplates", templ, dc.id)
                            templs.remove(templ)
                            if base not in templs:
                                LOG.info("Adding base template %s to %s.zDeviceTemplates", base, dc.id)
                                templs.append(base)
                        else:
                            LOG.warning("base template %s for template %s was not found, not migrating.")
                            continue
                        objchanged = True
                if objchanged:
