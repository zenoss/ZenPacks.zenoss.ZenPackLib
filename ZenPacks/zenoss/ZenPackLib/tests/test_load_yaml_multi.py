...
class TestMultiYAML(BaseTestCase):
    ...
    def test_multi_yaml(self):
        ...
        with tempfile.NamedTemporaryFile() as file_w:
            file_w.write(YAML_WHOLE.strip())
            file_w.flush()
            cfg_whole = zenpacklib.load_yaml(file_w.name)

        # reference yaml split across multiple files
        with tempfile.NamedTemporaryFile() as file_1:
            with tempfile.NamedTemporaryFile() as file_2:
                file_1.write(YAML_PART1.strip())
                file_1.flush()
                file_2.write(YAML_PART2.strip())
                file_2.flush()
                cfg_multi = zenpacklib.load_yaml_multi([file_1.name, file_2.name])
