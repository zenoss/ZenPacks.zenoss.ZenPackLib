...
class TestEscalateCountDefault(ZPLBaseTestCase):

    yaml_doc = YAML_DOC
    build = True

    def test_escalate_count_default(self):
        ...
        self.assertEquals(th.escalateCount, 0,
                          'Escalate Count expected {} ({}), got {} ({})'.format(0, 'int',
                                                                                th.escalateCount,
...
def test_suite():
    ...
    suite = TestSuite()
    suite.addTest(makeSuite(TestEscalateCountDefault))
    return suite
